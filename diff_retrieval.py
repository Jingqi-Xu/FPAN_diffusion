...
import utils_ret
from utils_ret import extract_features, extract_features_webdataset
import pickle
...
class SynthDataset(Dataset):
    def __init__(self, main_dir, transform, capjson=None):
        ...
        if capjson is None:
            ...
            self.total_imgs_sub = list(all_prompts_dict.keys())
            self.total_imgs = [f"{main_dir}/raw_images/{k}" for k in self.total_imgs_sub]
            self.prompts = [all_prompts_dict[k][0] for k in self.total_imgs_sub]
...
def Webdatasetloader(
    ...
    dataloader = dataset_to_dataloader(dataset, batch_size, num_prepro_workers, "webdataset")
    return dataloader
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if "laion" not in args.val_dir:
        ...
        data_loader_values = Webdatasetloader(
            input_dataset,
            size=256,
            batch_size=64,
            ...
            wds_image_key="jpg",
            wds_caption_key="txt",
            image_transform=ret_transform,
            cache_path=None,
        )
        #print(data_loader_values)
    '''
    ...
    if args.rank == 0:  # only rank 0 will work from now on
        ...
        if "laion" not in args.val_dir:
            ...
            values_features = extract_features_webdataset(args, model, data_loader_values, args.gpu, multiscale=args.multiscale)
        '''
        values_features = extract_features(args, model, data_loader_values, args.gpu, multiscale=args.multiscale)
        #print(values_features.size())
